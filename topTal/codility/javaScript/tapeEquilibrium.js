// A non-empty array A consisting of N integers is given. Array A r
// epresents numbers on a tape.
//
// Any integer P, such that 0 < P < N, splits this tape into two
// non-empty parts: A[0], A[1], ..., A[P − 1] and
// A[P], A[P + 1], ..., A[N − 1].
//
...
function tapeEquilibrium(A) {
  let beg = A[0];
  let rest = A.slice(1).reduce((acc, el) => acc + el) ;
  let min = Number.MAX_VALUE;

  for (let i = 1; i < A.length; i++) {
    min = Math.min(Math.abs(rest - beg), min);
    beg += A[i];
    rest -= A[i];
  }
