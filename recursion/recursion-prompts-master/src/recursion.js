...
var range = function(x, y) {
  let [a, b] = [Math.min(x, y), Math.max(x, y)];
  if (b - a < 2) return [];
  if (b - a === 2) return [b - 1];
...
var createArray = function(str) {
  ...
  let ans = [str[0]].concat(createArray(str.slice(1)));
  return ans;
};
...
var rMap = function(array, callback) {
  ...
  let prev = rMap(array.slice(0, array.length - 1), callback);
  return prev.concat(callback(array[array.length - 1]));
};
...
var replaceKeysInObj = function(obj, oldKey, newKey) {
  ...
  for (let key in obj) {
    if (typeof obj[key] === 'object') {
      return replaceKeysInObj(obj[key], oldKey, newKey);
    }
    if (key === oldKey) {
...
var nestedEvenSum = function(obj) {
  ...
  for (let o in obj) {
    if (obj[o].constructor === Object) {
      sum += nestedEvenSum(obj[o]);
    }
    if (obj[o] % 2 === 0) sum = obj[o];
  }
  return sum;
};
